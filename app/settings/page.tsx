...
export default function SettingsPage() {
  ...
  const handleTheme = (t: string) => {
    setTheme(t); localStorage.setItem("timesync-theme", t);
  ...
  async function handleReset() {
    ...
    localStorage.clear(); setTheme("system"); setTimerPreset("25/5"); setSound(true); setVibration(true); setTimerNotif(true); setMorningDigest(false); setHabitRemind(false); setShowCompleted(true); handleTheme("system");
  }
  ...
  return (
    <MotionMain className="p-4" variants={stagger} initial="initial" animate="animate">
      <MotionH1 variants={fadeUp} className="text-[28px] font-bold mb-6">Настройки</MotionH1>
      <MotionDiv variants={fadeUp} className="bg-white dark:bg-ios-card-dark rounded-2xl p-4 shadow-sm flex items-center gap-3 mb-6 border border-ios-separator/30">
      ...
      <Section title="Внешний вид">
        ...
        <div className="px-4 pb-3"><div className="flex bg-ios-bg dark:bg-white/10 rounded-xl p-1">
          {["Система", "Светлая", "Тёмная"].map((t) => { const key = t === "Система" ? "system" : t === "Светлая" ? "light" : "dark"; return (
            <button key={t} onClick={() => handleTheme(key)} className={`flex-1 py-1.5 text-sm rounded-lg transition-all ${theme === key ? "bg-white dark:bg-ios-card-dark shadow-sm font-medium" : "text-ios-gray"}`}>{t}</button>
          ); })}
        </div></div>
      </Section>
      ...
      <Section title="Помодоро таймер">
        ...
        <div className="px-4 pb-3"><div className="flex bg-ios-bg dark:bg-white/10 rounded-xl p-1">
          {["25/5", "52/17", "90/20"].map((p) => (
            <button key={p} onClick={() => { setTimerPreset(p); updateStorage("timesync-timer-preset", p); }} className={`flex-1 py-1.5 text-sm rounded-lg transition-all ${timerPreset === p ? "bg-white dark:bg-ios-card-dark shadow-sm font-medium" : "text-ios-gray"}`}>{p}</button>
          ))}
        </div></div>
      </Section>
