...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const t = localStorage.getItem("timesync-theme") || "system";
    const tp = localStorage.getItem("timesync-timer-preset") || "25/5";
    setTheme(t);
    setTimerPreset(tp);
  ...
  const updateStorage = (key: string, value: boolean | string) => {
  ...
  const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div className="mb-6">
      ...
      <div className="bg-white dark:bg-ios-card-dark rounded-2xl shadow-sm divide-y divide-ios-separator/50 border border-ios-separator/30">
        {children}
      </div>
    </div>
  );
  ...
  const Row = ({
    ...
    >
      <div className="w-8 h-8 rounded-lg bg-ios-bg dark:bg-white/10 flex items-center justify-center text-ios-gray flex-shrink-0">
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-base">{label}</p>
        {desc && <p className="text-xs text-ios-gray">{desc}</p>}
      </div>
      {action && <div className="flex-shrink-0">{action}</div>}
      {onClick && !action && <ChevronRight size={18} className="text-ios-gray flex-shrink-0" />}
    </div>
  );
  ...
  const Toggle = ({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) => (
    <button
      ...
      className={`w-12 h-7 rounded-full transition-colors relative ${value ? "bg-brand-green" : "bg-ios-separator"}`}
    >
      <div
  ...
  return (
    <main className="p-4">
      ...
      {/* Профиль */}
      <div className="bg-white dark:bg-ios-card-dark rounded-2xl p-4 shadow-sm flex items-center gap-3 mb-6 border border-ios-separator/30">
        <div className="w-12 h-12 rounded-full bg-black text-white flex items-center justify-center text-xl font-bold">
          {initial}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-base font-semibold">{name}</p>
          <p className="text-sm text-ios-gray truncate">{email}</p>
        </div>
      </div>
              ...
              return (
                <button
                  ...
                  className={`flex-1 py-1.5 text-sm rounded-lg transition-all ${
                    theme === key
                      ? "bg-white dark:bg-ios-card-dark shadow-sm font-medium"
                      : "text-ios-gray"
                  }`}
                >
                  {t}
      ...
      <Section title="Помодоро таймер">
        ...
        <div className="px-4 pb-3">
          <div className="flex bg-ios-bg dark:bg-white/10 rounded-xl p-1">
            {["25/5", "52/17", "90/20"].map((p) => (
              <button
                key={p}
                onClick={() => {
                  setTimerPreset(p);
                  updateStorage("timesync-timer-preset", p);
                }}
                className={`flex-1 py-1.5 text-sm rounded-lg transition-all ${
                  timerPreset === p
                    ? "bg-white dark:bg-ios-card-dark shadow-sm font-medium"
      ...
      <Section title="Звук и вибрация">
        <Row
          ...
          label="Звук таймера"
          desc="Звуковой сигнал по завершении"
          action={<Toggle value={sound} onChange={(v) => { setSound(v); updateStorage("timesync-sound", v); }} />}
        />
        <Row
          icon={<Smartphone size={18} />}
          label="Вибрация"
          desc="Вибросигнал по завершении"
          action={<Toggle value={vibration} onChange={(v) => { setVibration(v); updateStorage("timesync-vibration", v); }} />}
        />
      </Section>
      ...
      <Section title="Уведомления">
        <Row
          icon={<Bell size={18} />}
          label="Конец таймера"
          desc="Уведомление по завершении сессии"
          action={<Toggle value={timerNotif} onChange={(v) => { setTimerNotif(v); updateStorage("timesync-timer-notif", v); }} />}
        />
        <Row
          icon={<span className="text-sm">☀️</span>}
          label="Утренний дайджест"
          desc="Напоминание о задачах на день"
          action={<Toggle value={morningDigest} onChange={(v) => { setMorningDigest(v); updateStorage("timesync-morning", v); }} />}
        />
        <Row
          icon={<span className="text-sm">✓</span>}
          label="Привычки"
          desc="Напоминание отметить привычки"
          action={<Toggle value={habitRemind} onChange={(v) => { setHabitRemind(v); updateStorage("timesync-habit-remind", v); }} />}
        />
      </Section>
      ...
      <Section title="Задачи">
        <Row
          ...
          desc="Отображать завершённые задачи в списке"
          action={<Toggle value={showCompleted} onChange={(v) => { setShowCompleted(v); updateStorage("timesync-show-completed", v); }} />}
        />
      </Section>
