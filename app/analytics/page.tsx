...
function getLast7Days() {
  const days = [];
  for (let i = 6; i >= 0; i--) { const d = new Date(); d.setDate(d.getDate() - i); days.push({ date: d.toISOString().split("T")[0], label: d.toLocaleDateString("ru-RU", { weekday: "short" }) }); }
  return days;
}
  ...
  return (
    <main className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[{ label: "Выполнение", value: `${completionRate}%`, sub: `${doneTasks} из ${totalTasks}`, color: "text-brand-green" }, { label: "Фокус (всего)", value: `${Math.floor(totalFocusMin / 60)}ч ${totalFocusMin % 60}м`, sub: `${focus.length} сессий`, color: "text-brand-blue" }, { label: "Привычки", value: `${habitsDoneToday}/${habits.length}`, sub: "выполнено сегодня", color: "text-brand-orange" }, { label: "Стрик", value: `${overallMaxStreak}д`, sub: "макс. серия", color: "text-brand-purple" }].map((w, i) => (
      ...
      <div className={anim("animate-fade-up", 4)}>
        <div className="bg-white dark:bg-ios-card-dark rounded-2xl p-4 shadow-sm border border-ios-separator/30">
          ...
          <div className="space-y-3">
            {Object.entries(activeByPriority).map(([label, count]) => (<div key={label} className="flex items-center gap-3"><span className="text-sm w-16">{label}</span><div className="flex-1 h-2 bg-ios-bg dark:bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-ios-separator dark:bg-white/20 rounded-full transition-all duration-700" style={{ width: `${Math.min(100, count * 10)}%` }} /></div><span className="text-sm text-ios-gray w-6 text-right">{count}</span></div>))}
          </div>
        </div>
      </div>
