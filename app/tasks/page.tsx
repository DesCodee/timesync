...
import { AnimatePresence, MotionDiv, MotionMain, MotionSvg } from "@/components/motion";
...
export default function TasksPage() {
  ...
  async function fetchTasks() {
    if (!user) return;
    const { data } = await supabase.from("tasks").select("*, projects(name)").eq("user_id", user.id).order("created_at", { ascending: false });
    setTasks(data || []);
    setLoading(false);
  }
  ...
  useEffect(() => {
    if (user) { fetchTasks(); fetchProjects(); }
  }, [user]);
  ...
  async function createTask(e: React.FormEvent) {
    ...
    await supabase.from("tasks").insert({
      ...
      work_type: newWorkType || null, project_id: newProject || null,
      due_date: newDate || null, is_mit: newMit,
    });
    closeModal();
    fetchTasks();
  }
  ...
  function openEdit(task: Task) {
    ...
    setNewWorkType(task.work_type || "");
    setNewProject(task.project_id || "");
    setNewDate(task.due_date || "");
    setNewMit(task.is_mit);
    setShowModal(true);
  }
  ...
  return (
    <MotionMain className="p-4" variants={stagger} initial="initial" animate="animate">
      <MotionDiv variants={fadeUp} className="flex items-center justify-between mb-4">
        ...
        <button onClick={openCreate} className="bg-black text-white px-4 py-2 rounded-full text-sm font-medium flex items-center gap-1">
          <span>+</span> Новая
        </button>
      </MotionDiv>

      <MotionDiv variants={fadeUp} className="relative mb-4">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-ios-gray" size={18} />
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Поиск задач..."
      ...
      <MotionDiv variants={fadeUp} className="flex gap-2 mb-4 overflow-x-auto pb-1">
        {filters.map((f) => (
          <button key={f.key} onClick={() => setFilter(f.key)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${filter === f.key ? "bg-black text-white dark:bg-white dark:text-black" : "bg-white dark:bg-ios-card-dark text-ios-gray shadow-sm"}`}>
            {f.label}
      ...
      <MotionDiv variants={stagger} className="space-y-3">
        <AnimatePresence>
          {filtered.map((task) => (
            <MotionDiv key={task.id} layout variants={fadeUp} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white dark:bg-ios-card-dark rounded-2xl p-4 shadow-sm border border-ios-separator/30 group">
              <div className="flex items-start gap-3">
                <button onClick={() => toggleTask(task.id, task.is_completed)} className={`mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${task.is_completed ? "bg-brand-green border-brand-green" : "border-ios-gray"}`}>
                  {task.is_completed && (
                    <MotionSvg initial={{ scale: 0 }} animate={{ scale: 1 }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
      ...
      <AnimatePresence>
        {showModal && (
          <MotionDiv initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/50 backdrop-blur-sm p-0 md:p-4">
            <MotionDiv initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="bg-white dark:bg-ios-card-dark w-full max-w-md rounded-t-3xl md:rounded-3xl p-6 max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={editMode ? updateTask : createTask} className="space-y-4">
                <div>
                  ...
                  <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Что нужно сделать?" required className="w-full px-4 py-3 rounded-xl bg-ios-bg dark:bg-white/5 border-0 text-base focus:ring-2 focus:ring-black dark:focus:ring-white outline-none placeholder:text-ios-gray/60" />
                </div>
                <div>
                  <label className="text-xs font-medium text-ios-gray uppercase tracking-wider mb-1.5 block">Приоритет</label>
                  <div className="flex gap-2">
                    {["critical", "high", "medium", "low"].map((p) => (
                      <button key={p} type="button" onClick={() => setNewPriority(p)} className={`flex-1 py-2 rounded-xl text-xs font-medium capitalize ${newPriority === p ? priorityColors[p] : "bg-ios-bg dark:bg-white/5 text-ios-gray"}`}>{p}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <label className="text-xs font-medium text-ios-gray uppercase tracking-wider mb-1.5 block">Тип работы</label>
                  <select value={newWorkType} onChange={(e) => setNewWorkType(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-ios-bg dark:bg-white/5 border-0 text-base focus:ring-2 focus:ring-black dark:focus:ring-white outline-none">
                ...
                <div>
                  ...
                  <select value={newProject} onChange={(e) => setNewProject(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-ios-bg dark:bg-white/5 border-0 text-base focus:ring-2 focus:ring-black dark:focus:ring-white outline-none">
                ...
                <div>
                  ...
                  <input type="date" value={newDate} onChange={(e) => setNewDate(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-ios-bg dark:bg-white/5 border-0 text-base focus:ring-2 focus:ring-black dark:focus:ring-white outline-none" />
                </div>
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => setNewMit(!newMit)} className={`w-6 h-6 rounded border-2 flex items-center justify-center ${newMit ? "bg-brand-green border-brand-green" : "border-ios-gray"}`}>
                ...
                <button type="submit" className="w-full bg-black dark:bg-white text-white dark:text-black py-3.5 rounded-xl text-base font-medium">{editMode ? "Сохранить" : "Создать задачу"}</button>
              </form>
            </MotionDiv>
          </MotionDiv>
        )}
