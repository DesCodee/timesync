...
interface TimerState {
  preset: PresetKey; mode: Mode; timeLeft: number; isRunning: boolean;
  sessionsDone: number; selectedTask: string | null; sessionId: string | null;
...
function playTimerEndSound() {
  ...
  const audio = new Audio("data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBTGH0fPTgjMGHm7A7+OZURE");
  audio.play().catch(() => {});
}
...
export default function FocusPage() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("tasks").select("id, title").eq("user_id", user.id).eq("is_completed", false).order("created_at", { ascending: false }).then(({ data }) => setTasks(data || []));
  }, [user]);
    ...
    if (saved) {
      try {
        ...
        if (state.isRunning && state.endTime > now) {
          setPreset(state.preset); setMode(state.mode); setSessionsDone(state.sessionsDone);
          setSelectedTask(state.selectedTask); setSessionId(state.sessionId);
          setIsRunning(true); endTimeRef.current = state.endTime; startTimeRef.current = state.startTime;
  ...
  const saveState = useCallback(() => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({
      ...
      endTime: endTimeRef.current, startTime: startTimeRef.current,
    }));
  }, [preset, mode, timeLeft, isRunning, sessionsDone, selectedTask, sessionId]);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (remaining <= 0) { setTimeLeft(0); handleComplete(); } else { setTimeLeft(remaining); }
    };
  ...
  const handleCompleteFromState = (state: TimerState) => {
    setIsRunning(false);
    if (state.sessionId) { supabase.from("focus_sessions").update({ ended_at: new Date().toISOString() }).eq("id", state.sessionId); setSessionId(null); }
    if (state.mode === "work") {
      const newSessions = state.sessionsDone + 1; setSessionsDone(newSessions);
      if (newSessions >= 4) { setMode("longBreak"); setTimeLeft(PRESETS[state.preset].longBreak * 60); setSessionsDone(0); }
      else { setMode("break"); setTimeLeft(PRESETS[state.preset].break * 60); }
    } else { setMode("work"); setTimeLeft(PRESETS[state.preset].work * 60); }
  ...
  const handleComplete = useCallback(() => {
    ...
    playTimerEndSound();
    vibrateTimerEnd();
    if (sessionId) { supabase.from("focus_sessions").update({ ended_at: new Date().toISOString(), actual_duration_min: totalTime }).eq("id", sessionId); setSessionId(null); }
    if (mode === "work") {
      const newSessions = sessionsDone + 1; setSessionsDone(newSessions);
      if (newSessions >= 4) { setMode("longBreak"); setTimeLeft(currentPreset.longBreak * 60); setSessionsDone(0); }
      else { setMode("break"); setTimeLeft(currentPreset.break * 60); }
    } else { setMode("work"); setTimeLeft(currentPreset.work * 60); }
    localStorage.removeItem(STORAGE_KEY);
  }, [mode, sessionsDone, sessionId, totalTime, currentPreset]);

  async function toggleTimer() {
    if (!isRunning) {
      const now = Date.now(); endTimeRef.current = now + timeLeft * 1000; startTimeRef.current = now; setIsRunning(true);
      if (mode === "work" && user) {
      ...
      setIsRunning(false); const elapsed = Math.round((Date.now() - startTimeRef.current) / 60000);
      if (sessionId) { supabase.from("focus_sessions").update({ actual_duration_min: elapsed || 1 }).eq("id", sessionId); }
      setTimeLeft(Math.ceil((endTimeRef.current - Date.now()) / 1000));
    }
  }

  function stopTimer() {
    setIsRunning(false); if (intervalRef.current) clearInterval(intervalRef.current);
    if (sessionId) { const elapsed = Math.round((Date.now() - startTimeRef.current) / 60000); supabase.from("focus_sessions").update({ ended_at: new Date().toISOString(), actual_duration_min: elapsed || 1 }).eq("id", sessionId); setSessionId(null); }
    setMode("work"); setTimeLeft(currentPreset.work * 60); setSessionsDone(0); localStorage.removeItem(STORAGE_KEY);
  }

  function resetTimer() {
    setIsRunning(false); if (intervalRef.current) clearInterval(intervalRef.current); setMode("work"); setTimeLeft(currentPreset.work * 60); setSessionsDone(0);
    if (sessionId) { supabase.from("focus_sessions").update({ ended_at: new Date().toISOString() }).eq("id", sessionId); setSessionId(null); }
    localStorage.removeItem(STORAGE_KEY);
  }

  function switchPreset(p: PresetKey) {
    setPreset(p); setIsRunning(false); setMode("work"); setTimeLeft(PRESETS[p].work * 60); setSessionsDone(0);
    if (sessionId) { supabase.from("focus_sessions").update({ ended_at: new Date().toISOString() }).eq("id", sessionId); setSessionId(null); }
    localStorage.removeItem(STORAGE_KEY);
  }
  ...
  return (
    <main className="p-4 flex flex-col items-center">
      ...
      <div className="flex items-center gap-6 mb-10">
        <button onClick={resetTimer} className="w-14 h-14 rounded-full bg-white dark:bg-ios-card-dark flex items-center justify-center shadow-sm border border-ios-separator/30"><RotateCcw size={22} /></button>
        ...
        <button onClick={stopTimer} className="w-14 h-14 rounded-full bg-white dark:bg-ios-card-dark flex items-center justify-center shadow-sm border border-ios-separator/30"><Square size={22} fill="currentColor" /></button>
      </div>
