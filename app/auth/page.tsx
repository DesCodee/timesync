    ...
    <main className="min-h-screen flex items-center justify-center p-4 bg-ios-bg dark:bg-ios-dark">
      <div className="w-full max-w-sm space-y-6 animate-scale-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 animate-fade-up">
          {!isLogin && <div className="relative"><User className="absolute left-3 top-1/2 -translate-y-1/2 text-ios-gray" size={18} /><input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("auth", "name")} className="w-full pl-10 pr-4 py-3 rounded-2xl bg-white dark:bg-ios-card-dark border-0 text-base outline-none placeholder:text-ios-gray/60 shadow-sm" /></div>}
          ...
          <div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-ios-gray" size={18} /><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t("auth", "password")} required className="w-full pl-10 pr-4 py-3 rounded-2xl bg-white dark:bg-ios-card-dark border-0 text-base outline-none placeholder:text-ios-gray/60 shadow-sm" /></div>
          ...
          <button type="submit" disabled={loading} className="w-full bg-black dark:bg-white text-white dark:text-black py-3.5 rounded-2xl text-base font-medium active:scale-[0.98] transition-transform disabled:opacity-50 flex items-center justify-center gap-2">
            {loading ? t("auth", "loading") : isLogin ? t("auth", "submitLogin") : t("auth", "submitRegister")} {!loading && <ArrowRight size={18} />}
          </button>
        </form>
