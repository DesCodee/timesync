    ...
    <main className="min-h-screen flex items-center justify-center p-4 bg-ios-bg dark:bg-ios-dark">
      <div className="w-full max-w-sm space-y-6 animate-scale-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 animate-fade-up">
          ...
          {mode !== "forgot" && <div className="relative"><Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-ios-gray" size={18} /><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t("auth", "password")} required={mode !== "forgot"} className="w-full pl-10 pr-4 py-3 rounded-2xl bg-white dark:bg-ios-card-dark border-0 text-base outline-none placeholder:text-ios-gray/60 shadow-sm" /></div>}
